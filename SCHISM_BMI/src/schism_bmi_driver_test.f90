...
program schism_driver_test
  ...
  !---------------------------------------------------------------------
    ...
    double precision, allocatable            :: Q_bnd_t0(:), Q_bnd_t1(:), ETA2_bnd_t0(:), ETA2_bnd_t1(:) ! Boundary condition terms
  ...
  allocate(Q_bnd_t0(size(ieg_source)))
  allocate(Q_bnd_t1(size(ieg_source)))
  ...
  Q_bnd_t0(:) = 0.25
  status = m%set_value('Q_bnd_t0', Q_bnd_t0)
  ...
  Q_bnd_t1(:) = 0.5
  status = m%set_value('Q_bnd_t1', Q_bnd_t1)
  ...
  !---------------------------------------------------------------------
    ...
    status = m%get_var_grid('Q_bnd_t1', grid_int)
    print*, "The integer value for the ", 'Q_bnd_t1', " grid is ", grid_int

    ! get_grid_type
    status = m%get_grid_type(grid_int, grid_type)
    print*, "The grid type for ", 'Q_bnd_t1', " is ", trim(grid_type)

    ! get_grid_rank
    status = m%get_grid_rank(grid_int, grid_rank)
    print*, "The grid rank for ", 'Q_bnd_t1', " is ", grid_rank
