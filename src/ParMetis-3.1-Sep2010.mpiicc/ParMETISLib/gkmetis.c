 ...
 * George
 *
 * $Id: gkmetis.c,v 1.1.2.1 2005/04/04 17:06:23 tjcamp Exp $
 *
 */
...
void ParMETIS_V3_PartGeomKway(idxtype *vtxdist, idxtype *xadj, idxtype *adjncy,
	      ...
	      int *options, int *edgecut, idxtype *part, MPI_Comm *commF)
{
  int h, i, j;
  ...
  float *itpwgts, iubvec[MAXNCON];
   ...
   /* start fortran MPI_Comm integer to C struct translation */ 
   MPI_Comm comm_world;
   MPI_Comm *comm;
   comm_world = MPI_Comm_f2c((MPI_Fint)*commF);
   comm       = &comm_world;
   /* end   fortran MPI_Comm integer to C struct translation */ 
