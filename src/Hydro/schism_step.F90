...
!-------------------------------------------------------------------------------
  ...
  !$OMP do
        do i=1,npa
          ...
          do j=1,ntip
            ...
            arg=tfreq(j)*time-real(ncyc,rkind)*2.d0*pi+jspc(j)*xlon_o(i)+tear(j)
  ...
  !$OMP end parallel
        ...
        do i=1,npa
          if(lhas_ice(i)) then
            ...
            maxpice=min(maxpice,5*rho0*grav)
  ...
  !            swild10_tmp(1:3,1:3) = swild10(1:3,1:3)
              if(xt.ne.xt) write(*,*)nnel,xt,yt,xctr2,yctr2,eps
  ...
  !      enddo !i
        ...
        if(mod(it,nspool)==0) then
          ...
          call writeout_nc(id_out_var(3),'area_elem',4,1,nea,dble(area))
