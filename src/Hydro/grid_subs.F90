...
!  if(allocated(iselfgb)) deallocate(iselfgb)
  ...
  if(allocated(xlon_o)) deallocate(xlon_o); allocate(xlon_o(npa),stat=stat);
  if(stat/=0) call parallel_abort('AQUIRE_HGRID: xlon_o allocation failure')
  ...
  if(allocated(ylat_o)) deallocate(ylat_o); allocate(ylat_o(npa),stat=stat);
  if(stat/=0) call parallel_abort('AQUIRE_HGRID: ylat_o allocation failure')

  open(14,file=in_dir(1:len_in_dir)//'hgrid2.gr3',status='old',iostat=stat)
  if(stat/=0) call parallel_abort('AQUIRE_HGRID: open(14,hgrid2) failure')
  read(14,*); read(14,*);
  do i=1,np_global
    ...
    if(node%rank==myrank) then
      ...
      if(ics==1) then
        ...
        xlon_o(ii)=xtmp*deg2rad
        ylat_o(ii)=ytmp*deg2rad
        dp(ii)=dptmp
  ...
  open(14,file=in_dir(1:len_in_dir)//'hgrid.gr3',status='old',iostat=stat)
  if(stat/=0) call parallel_abort('AQUIRE_HGRID: open(14,hgrid1) failure')
  read(14,*); read(14,*);
  do i=1,np_global
    read(14,*) ipgb,xtmp,ytmp,dptmp
    node=>ipgl(ipgb)
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  do ie=1,nea
    ...
    !write(12,*) ie,xctr(ie),xel(1,ie),xel(2,ie),xel(3,ie),area(ie)
