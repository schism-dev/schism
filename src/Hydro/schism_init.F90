...
!     Local variables
      ...
      real(rkind) :: cwtmp2,wtmp1,tmp,slam0,sfea0,rlatitude,coricoef,dfv0,dfh0, &
                    ...
                    &et,qq,tr,ft1,dep,wtratio,shapiro0,rmaxvel,tf
...
!     All other arrays
      allocate(sdbt(2+ntracers,nvrt,nsa), & !webt(nvrt,nea), bubt(2,nea), & 
         ...
         &  hdif(nvrt,npa),shapiro(nsa),d2uv(2,nvrt,nsa),fluxprc(npa),fluxevp(npa),prec_snow(npa), &
         &  prec_rain(npa) ,& 
...
#ifdef USE_MICE
        allocate(tau_oi(2,npa),fresh_wa_flux(npa),net_heat_flux(npa),ice_evap(npa),lhas_ice(npa),stat=istat)
...
!     Initialize some arrays and constants
      tempmin=-2.0d0; tempmax=40.d0; saltmin=0.d0; saltmax=42.d0
...
!'
        ...
        do i=1,np_global
          ...
          if(ipgl(i)%rank==myrank) then
            ii=ipgl(i)%id
            xlon_o(ii)=xtmp*pi/180.d0
            ylat_o(ii)=ytmp*pi/180.d0
            !Pre-compute species function to save time
            fun_lat(0,ii)=3.d0*sin(ylat_o(ii))**2.d0-1.d0
            fun_lat(1,ii)=sin(2.d0*ylat_o(ii))
            fun_lat(2,ii)=cos(ylat_o(ii))**2.d0
          endif
        enddo !i
        close(32)
        lreadll=.true.
...
!'
        ...
        do i=1,np_global
          read(32,*)j,xtmp,ytmp
          if(ipgl(i)%rank==myrank) then
            xlon_o(ipgl(i)%id)=xtmp*pi/180.d0
            ylat_o(ipgl(i)%id)=ytmp*pi/180.d0
          endif
        enddo !i
        close(32)
        lreadll=.true.
...
!$OMP parallel do default(shared) private(i,id1,id2,sphi)
        do i=1,nsa
          id1=isidenode(1,i)
          id2=isidenode(2,i)
          sphi=(ylat_o(id1)+ylat_o(id2))/2.d0
          if(ics==1) then
            cori(i)=fc+beta*(sphi-sfea0)
          else !ics=2
            cori(i)=2.d0*omega_e*sin(sphi)
          endif !ics
          if(myrank==0) write(31,*)i,xlon_o(id1)/pi*180,ylat_o(id1)/pi*180,cori(i)
        enddo !i=1,nsa
!$OMP end parallel do
...
!     Wind 
      if(nws>=2.and.nws<=3) then !CORIE mode; read in hgrid.ll and open debug outputs
        ...
        do i=1,np_global
          read(32,*)j,tmp1,tmp2
          if(ipgl(i)%rank==myrank) then
            xlon_o(ipgl(i)%id)=tmp1*pi/180.d0
            ylat_o(ipgl(i)%id)=tmp2*pi/180.d0
          endif
        enddo !i
        close(32)
        lreadll=.true.
...
!...  otherwise they will be assigned values below.
      ...
      if(myrank==0) write(16,*)'start init ice...'
      call ice_init
