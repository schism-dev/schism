        ...
        if(myrank==0) then
          if(iout_sta==1) then !ASCII
            ...
            if(ihot<=1) then
              ...
              j=nf90_def_dim(ncid_sta,'nTracers',ntracers,nwild(3))
              j=nf90_def_dim(ncid_sta,'time',NF90_UNLIMITED,nwild(4))

              j=nf90_def_var(ncid_sta,'time_in_sec',NF90_FLOAT,nwild(4),id_staout(1))
              ...
              j=nf90_def_var(ncid_sta,'uvel',NF90_FLOAT,nwild2(1:2),id_staout(6))
              j=nf90_def_var(ncid_sta,'vvel',NF90_FLOAT,nwild2(1:2),id_staout(7))
              j=nf90_def_var(ncid_sta,'wvel',NF90_FLOAT,nwild2(1:2),id_staout(8))
              j=nf90_def_var(ncid_sta,'zcoordinate',NF90_FLOAT,nwild2(1:3),id_staout(9))
              nwild2(1:3)=nwild((/3,1,4/))
              do i=1,ntracers
                j=nf90_def_var(ncid_sta,'tracers',NF90_FLOAT,nwild2(1:3),id_staout(9+i))
              enddo !i
              nwild2(1:3)=nwild((/2,1,4/))

              j=nf90_def_var(ncid_sta,'uvel3D',NF90_FLOAT,nwild2(1:3),id_staout(10))
              j=nf90_def_var(ncid_sta,'vvel3D',NF90_FLOAT,nwild2(1:3),id_staout(11))

              nwild2(1:4)=nwild((/3,2,1,4/))
              j=nf90_def_var(ncid_sta,'tracers3D',NF90_FLOAT,nwild2(1:4),id_staout(12))

              !deflate
              do i=1,13
                j=nf90_def_var_deflate(ncid_sta,id_staout(i),0,1,4)
              enddo !i
