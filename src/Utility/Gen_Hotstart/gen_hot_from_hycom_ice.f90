...
! Ice part added by Qian Wang (Feb 2025)
...
! ifort -O2 -mcmodel=medium -assume byterecl -CB -g -traceback -o gen_hot_from_hycom_ice ../UtilLib/schism_geometry.f90 \
! ../UtilLib/extract_mod.f90 ../UtilLib/compute_zcor.f90 ../UtilLib/pt_in_poly_test.f90 gen_hot_from_hycom_ice.f90 \
...
!      integer :: iargc
      ...
      integer :: ixlen, iylen, ilen, ixlen0 ! sampled lengths in each coordinate direction
...
!     V-grid
     ...
     &eout_tmp(np),tsel(2,nvrt,ne),zeros(nvrt,max(ne,np)),siuout(np),sivout(np),&
     &sicout(np),sihout(np),stat=istat)
...
!     Open nc file 
      ...
      status = nf90_open('ICE_'//char3(1:len_char3)//'.nc', nf90_nowrite, ncids(3))

      if(ifile==1) then
!       Get dimnesions from S
        status = nf90_inq_varid(sid, "salinity", svid)
        print*, 'Done reading variable ID'
        print*, sid,svid,ncids(1:3),'TS_'//char3(1:len_char3)//'.nc'
...
!        allocate(hnc(ixlen,iylen))
        ...
        allocate(sic(ixlen,iylen),stat=ier)
        allocate(sih(ixlen,iylen),stat=ier)
        allocate(siu(ixlen,iylen),stat=ier)
        allocate(siv(ixlen,iylen),stat=ier)
        uvel=0; vvel=0; ssh=0; sic=0; siv=0; siu=0; siv=0
  
!       get static info (lat/lon grids etc) 
        status = nf90_inq_varid(ncids(1), "lon", xvid)
        status = nf90_get_var(ncids(1), xvid, xind(1:ixlen0))
...
!       processing static info
        do i=1,ixlen0
          lon(i,:)=xind(i)
          if(i<ixlen0) then; if(xind(i)>=xind(i+1)) then
...
!       Get rest of varid's
        ...
        status = nf90_inq_varid(ncids(3), "sic", icvid)
        status = nf90_inq_varid(ncids(3), "sih", ihvid)
        status = nf90_inq_varid(ncids(3), "siu", iuvid)
        status = nf90_inq_varid(ncids(3), "siv", ivvid)
...
!                at bottom, ilen at surface; revert if necessary!
        status = nf90_get_var(sid,svid,salt(1:ixlen0,:,ilen:1:-1),start=(/1,1,1,it2/),count=(/ixlen0,iylen,ilen,1/))
        status = nf90_get_var(sid,tvid,temp(1:ixlen0,:,ilen:1:-1),start=(/1,1,1,it2/),count=(/ixlen0,iylen,ilen,1/))
        status = nf90_get_var(ncids(2),uvid,uvel(1:ixlen0,:,ilen:1:-1),start=(/1,1,1,it2/),count=(/ixlen0,iylen,ilen,1/))
        status = nf90_get_var(ncids(2),vvid,vvel(1:ixlen0,:,ilen:1:-1),start=(/1,1,1,it2/),count=(/ixlen0,iylen,ilen,1/))
        status = nf90_get_var(ncids(1),evid,ssh(1:ixlen0,:),start=(/1,1,it2/),count=(/ixlen0,iylen,1/))
        status = nf90_get_var(ncids(3),icvid,sic(1:ixlen0,:),start=(/1,1,it2/),count=(/ixlen0,iylen,1/))
        status = nf90_get_var(ncids(3),ihvid,sih(1:ixlen0,:),start=(/1,1,it2/),count=(/ixlen0,iylen,1/))
        status = nf90_get_var(ncids(3),iuvid,siu(1:ixlen0,:),start=(/1,1,it2/),count=(/ixlen0,iylen,1/))
        status = nf90_get_var(ncids(3),ivvid,siv(1:ixlen0,:),start=(/1,1,it2/),count=(/ixlen0,iylen,1/))
        ...
        rjunk=-3.e4*1.e-3+20
!repeat, var at 180 equal to at -180
        ssh(ixlen,:) = ssh(1,:)
        sic(ixlen,:) = sic(1,:)
        sih(ixlen,:) = sih(1,:)
        siu(ixlen,:) = siu(1,:)
...
!             if(salt(i,j,ilen)<rjunk+0.1) then  
            if(sih(i,j)<-3+0.1) then !use sic to judge
              sic(i,j) = 0 !ice free
              sih(i,j) = 0
              siu(i,j) = 0
              siv(i,j) = 0
...
!       Test outputs
        ...
        do i=1,ixlen
          do j=1,iylen
            ...
            !write(101,*)icount,lon(i,j),lat(i,j),sic(i,j),sih(i,j)
          enddo !j
        enddo !i
...
!              write(18,*)i,k,ix,iy,lev,vrat,kbp(ix,iy)
              ...
              sicout(i)=sic(ix,iy)*arco(1,i)+sic(ix+1,iy)*arco(2,i)+sic(ix+1,iy+1)*arco(3,i)+sic(ix,iy+1)*arco(4,i)
              sihout(i)=sih(ix,iy)*arco(1,i)+sih(ix+1,iy)*arco(2,i)+sih(ix+1,iy+1)*arco(3,i)+sih(ix,iy+1)*arco(4,i)
              siuout(i)=siu(ix,iy)*arco(1,i)+siu(ix+1,iy)*arco(2,i)+siu(ix+1,iy+1)*arco(3,i)+siu(ix,iy+1)*arco(4,i)
              sivout(i)=siv(ix,iy)*arco(1,i)+siv(ix+1,iy)*arco(2,i)+siv(ix+1,iy+1)*arco(3,i)+siv(ix,iy+1)*arco(4,i)
...
!         Debug
          do i=1,np
            ...
            write(28,*)i,xl(i),yl(i),sicout(i),sihout(i),siuout(i),sivout(i)
...
!            enddo !k
          ...
          iret=nf90_def_var(ncids(4),'sic2',NF90_DOUBLE,var1d_dims,ivarid(22))
          iret=nf90_def_var(ncids(4),'sih2',NF90_DOUBLE,var1d_dims,ivarid(23))
          iret=nf90_def_var(ncids(4),'siu2',NF90_DOUBLE,var1d_dims,ivarid(24))
          iret=nf90_def_var(ncids(4),'siv2',NF90_DOUBLE,var1d_dims,ivarid(25))
          ...
          !iret=nf90_put_var(ncids(4),ivarid(1),dble(44496000.))
          !iret=nf90_put_var(ncids(4),ivarid(2),444960)
          !iret=nf90_put_var(ncids(4),ivarid(3),516)
          !iret=nf90_put_var(ncids(4),ivarid(20),444960)
          ...
          iret=nf90_put_var(ncids(4),ivarid(22),dble(sicout(1:np)),(/1/),(/np/)) !sea ice concerntration
          iret=nf90_put_var(ncids(4),ivarid(23),dble(sihout(1:np)),(/1/),(/np/)) !sea ice thickness
          iret=nf90_put_var(ncids(4),ivarid(24),dble(siuout(1:np)),(/1/),(/np/)) !sea ice u
          iret=nf90_put_var(ncids(4),ivarid(25),dble(sivout(1:np)),(/1/),(/np/)) !sea ice v
