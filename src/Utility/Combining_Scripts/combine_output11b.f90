!Added filtering of elev for hydrologic cases
!===============================================================================
...
!  ifort -cpp -O2 -assume byterecl -o combine_output11b.exe ../UtilLib/argparse.f90 ../UtilLib/schism_geometry.f90 combine_output11b.f90 -I$NETCDF/include -I$NETCDF_FORTRAN/include -L$NETCDF_FORTRAN/lib -L$NETCDF/lib -lnetcdf -lnetcdff
...
!  character(len=30),dimension(nfile)   :: files  !< base names (e.g. elev.61) of files to combine
  ...
  character(len=4) :: a_4
  integer,allocatable :: elnode(:,:),nne(:),indel(:,:)
...
! Read local_to_global_0000 for global info
  ...
  &idry(np_global),idry_e(ne_global),idry_s(ns_global),nne(np_global),stat=istat)
...
!    enddo !m
  ...
  enddo !i 
  mnei=maxval(nne)
  allocate(indel(mnei,np_global),stat=istat)
  nne(:)=0
  do i=1,ne_global
    do j=1,i34(i)
      nd=elnode(j,i)
...
!       Filter elev for isolated wet nodes
        if(trim(adjustl(variable_nm(m))).eq.'elev') then
!          write(99,*)'Filtering elev..',m,ispool,iinput
          do i=1,np_global
            ifl_dry=1 !init
            do j=1,nne(i)
              ie=indel(j,i)
            ...
            enddo !j
            if(ifl_dry==1) then !enforce limit
              outd(m)%data(1,1,i)=min(-dp(i),outd(m)%data(1,1,i))
            endif
          enddo !i
        endif !trim
...
!        endif !i23d
  ...
  &xcj,ycj,dps,kbs,idry,idry_s,idry_e,nne,indel)
