/* $Id: noxprotos.h,v 1.3 2004/03/23 05:15:07 pturner Exp $
 *
...
void putbarlegend(int i,	/* which set */
	        ...
	        int fc,		/* fill color */
	        int fp);	/* fill pattern */
int putlegendrect(int fill, int fillusing, int fillcolor, int fillpat, int cy, int wy, int ly);
...
int readnetcdf(int gno, int setno, char *netcdfname, char *xvar, 
		char *yvar, int start, int stop, int stride, int index2d);
...
void drawboxcolor(plotarr p, Isolparms isol);
...
void puthersh(int xpos, int ypos, double scale, int dir, int just, int color, int (*vector) (), char *s);
