...
#ifndef lint
static char RCSid[] = "$Id: sigmaztrans.c,v 1.5 2009/05/11 16:44:59 pturner Exp $";
#endif
...
XtCallbackProc SigmaZTransAccept(Widget w, XtPointer clid, XtPointer calld);
...
typedef struct {
    ...
    Widget displayg;
    Widget *displayv;
} SigmaZTransDisplayUI;

void SigmaZTransDisplayFrame(Widget w, XtPointer clid, XtPointer calld);
XtCallbackProc SigmaZTransDisplayAccept(Widget w, XtPointer clid, XtPointer calld);
XtCallbackProc SigmaZTransDisplayUpdate(Widget w, XtPointer clid, XtPointer calld);
XtCallbackProc SigmaZTransIsolinesAccept(Widget w, XtPointer clid, XtPointer calld);

XtCallbackProc SigmaZTransDisplayUpdate(Widget w, XtPointer clid, XtPointer calld)
{
    int disp, trans, color;
    SigmaZTransDisplayUI *a = (SigmaZTransDisplayUI *) clid;
    trans = GetChoice(a->trans);
    color = g[cg].trans[trans].p.color;
    SetChoice(a->color, color);
}
...
XtCallbackProc SigmaZTransDisplayAccept(Widget w, XtPointer clid, XtPointer calld)
{
    ...
    g[cg].trans[trans].p.color = color;
...
XtCallbackProc SigmaZTransIsolinesAccept(Widget w, XtPointer clid, XtPointer calld)
{
	...
	create_isolines_popup("SigmaZTransect Isolines", &g[cg].trans[n].ip, 1);
    } else {
    }
}
...
void SigmaZTransDisplayFrame(Widget w, XtPointer clid, XtPointer calld)
{
    static SigmaZTransDisplayUI ui;
    Arg al[8];
    Widget panel, wbut, fr, bb, sep, rc, rc1, rc2;
    int ac = 0, i;
    setistop();
    if (!ui.top) {
	ui.top = XmCreateDialogShell(app_shell, "SELFE SigmaZTransect Display", NULL, 0);
	...
	ui.color = CreateColorChoice(panel, "Display vectors using color:", 1);
	...
	XtAddCallback(wbut, XmNactivateCallback, (XtCallbackProc) SigmaZTransDisplayAccept, (XtPointer) & ui);
	wbut = XtVaCreateManagedWidget("Isolines of scalars...", xmPushButtonWidgetClass, rc, NULL);
	XtAddCallback(wbut, XmNactivateCallback, (XtCallbackProc) SigmaZTransIsolinesAccept, (XtPointer) & ui);
    ...
    SigmaZTransDisplayUpdate(NULL, (XtPointer) & ui, NULL);
    XtRaise(ui.top);
}

XtCallbackProc SigmaZTransAccept(Widget w, XtPointer clid, XtPointer calld)
{
    SigmaZTransectUI *a = (SigmaZTransectUI *) clid;
    int i;
    int datatype;
    int flowno;
    int gno;
	...
	if (!yesno("SigmaZTransect is active, kill it?", " ", " YES ", " NO ")) {
	    return;
	} else {
...
/* here check for active grid and flow # */
    ...
    SigmaZTransDisplayFrame(NULL, NULL, NULL);
}

void SigmaZTransFrame(Widget w, XtPointer clid, XtPointer calld)
{
    static SigmaZTransectUI ui;
    Arg al[8];
    Widget panel, wbut, fr, bb, sep, rc, rc1, rc2;
    int ac = 0, i;
    setistop();
	...
	XtVaSetValues(rc, XmNorientation, XmHORIZONTAL, NULL);
	wbut = XtVaCreateManagedWidget("Accept", xmPushButtonWidgetClass, rc, NULL);
	XtAddCallback(wbut, XmNactivateCallback, (XtCallbackProc) SigmaZTransAccept, (XtPointer) & ui);
	wbut = XtVaCreateManagedWidget("Done", xmPushButtonWidgetClass, rc, NULL);
	XtAddCallback(wbut, XmNactivateCallback, (XtCallbackProc) destroy_dialog, ui.top);
	XtManageChild(rc);
	XtManageChild(panel);
...
int ReadSigmaZTrans(Transect * tr, int append)
{
    char fname[1024];
    int first = 1;
...
/* get the bottom index for this point */
	    binds[0] = h1.bi[nn[0]];
	    binds[1] = h1.bi[nn[1]];
	    binds[2] = h1.bi[nn[2]];
	    iminmax(3, binds, &bind, &sind);
	    p[cnt].bi = bind;
	    p[cnt].si = h1.nvrt - 1;
...
/* get depths */
	    ElioInterpTimeStep(&h1, p[j].elem, p[j].x, p[j].y, p[j].h, &t1, binds, sinds, depths);
/* get data */
	    ElioInterpTimeStep(&h2, p[j].elem, p[j].x, p[j].y, p[j].h, &t2, binds, sinds, data);
	    if (h2.ivs == 2) {
		ElioInterpTimeStep(&h3, p[j].elem, p[j].x, p[j].y, p[j].h, &t3, binds, sinds, vv);
...
/* need to have at least 2 nodes for all this to work */
	    if (j > 0) {
/* here, need to make special bottom elements */
		if (p[j].bi == p[j - 1].bi) {	// both equal
//printf("Equal %d\n", p[j].bi);

		    for (k = p[j].bi; k < h1.nvrt - 1; k++) {
