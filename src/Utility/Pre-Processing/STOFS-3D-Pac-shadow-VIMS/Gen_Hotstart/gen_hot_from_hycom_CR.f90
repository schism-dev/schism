...
! Generate hotstart.nc from gridded HYCOM data (nc file)
...
! Charles Seaton: this version uses two control points in estuary.gr3 (-2 and -1) to generate a temperature and 
! salinity gradient along the x-axis between the two points within the estuary region, between fresh and HYCOM values, 
! specifically to prevent shocks in the Columbia River estuary. Could be generalized to handle multiple estuaries and directions 
! other than east-west, but Columbia River estuary is the only one where steep horizontal gradient shocks are a known problem
...
!     (6) HYCOM files: [SSH,TS,UV]_1.nc (beware scaling etc), uses first timestep in file
!   Output: hotstart.nc
!   Debug outputs: fort.11 (fatal errors); fort.20 (warning);
!   Use note: hotstart standalone version is useful if the domain is large 

! ifort -O2 -mcmodel=medium -assume byterecl -CB -o gen_hot_from_hycom.exe ../UtilLib/schism_geometry.f90 \
! ../UtilLib/extract_mod.f90 ../UtilLib/compute_zcor.f90 ../UtilLib/pt_in_poly_test.f90 gen_hot_from_hycom_CR.f90 \
...
!      read(10,*) dtout !time step in .nc [sec]
      nob=0
      nndays=0 !# of days needed in output
      nfiles=1 !# of stacks of HYCOM files
...
!     Read in hgrid and vgrid
      ...
      do i=1,np
        ...
        iest(i)=nint(tmp)
        if(iest(i)/=0.and.iest(i)/=1.and.iest(i)/=-1.and.iest(i)/=-2) then
      ...
      enddo !i
!     Open bnds not needed for hotstart.nc
!      read(14,*) nope
!      read(14,*) neta
!      ntot=0
...
!       Do sth for 1st step
        if(ifile==istart_day.and.it2==ilo) then
          ...
          do i=1,ixlen
            do j=1,iylen
              if(kbp(i,j)==-1) then !invalid pts
                ...
                loop6: do  ! outward spiral search
                  m=m+1
                  i3 = i +  max(-m,1-i)! lowest i3
                ...
                !    print*, i,j,m,i3,j3
                  enddo !ii
                  do ii=max(-m,1-i)+1,min(m,ixlen-i)-1 ! skip first
                    ! and last ii, handled in j loops
                    i3=max(1,min(ixlen,i+ii))
                    j3 = j + max(-m,1-j) ! lowest j3
                    if(kbp(i3,j3)>0) then !found
                      i1=i3; j1=j3
                      exit loop6
                    endif
                 !   print*, i,j,m,i3,j3
                    j3 = j + min(m,iylen-j) ! highest j3
                    if(kbp(i3,j3)>0) then !found
                      i1=i3; j1=j3
                      exit loop6
                    endif
...
!                   Find a triangle
        ...
        else !skip to extend, ! this section handled 3Dth files, and is never reached in this version of the code
          do i=1,ixlen
            do j=1,iylen
...
!          vvel0=vvel
          ...
          call flush(20)
        endif !ifile==istart_day.and.it2==
...
!              endif
              ...
              if(tempout(k,i)<tempmin.or.tempout(k,i)>tempmax.or. &
                ...
                write(11,*)'Interpolated values invalid:',i,k,'T',tempout(k,i),'S',saltout(k,i),uout(k,i), &
     &vout(k,i),eout(i),ix, iy, lev, temp(ix,iy,lev), salt(ix,iy,lev), ifile, it2
                stop
              endif
...
!            endif
        ...
        enddo !ii
        if(ifile==istart_day.and.it2==ilo) then ! modify estuary nodes, needs to
! be done after ianchor nodes have been processed above
          do i=1,iup
            if(iest(i)==1) then
               if(ianchor1>-1) then
                ...
                do k=1,nvrt
                  tempout(k,i)=tem_es*xrat + tempout(k,ianchor1)*(1-xrat)
                  saltout(k,i)=sal_es*xrat + saltout(k,ianchor1)*(1-xrat)
                enddo !k=1,nvrt
                write(101,*) i,iest(i),xl(i), xl(ianchor2),xl(ianchor1), xrat, saltout(1,i), tempout(1,i)
             endif !iest
          enddo !ii
        endif !ixy(i,1)==0.or.
