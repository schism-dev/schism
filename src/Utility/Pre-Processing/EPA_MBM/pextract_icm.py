...
#-----------------------------------------------------------------------------
run='/sciclone/data10/wangzg/CBP/RUN17k'
...
iplot=0          #run plot_icm.py after exacting the data

#hpc resource requst
walltime='02:00:00'; nnode=2;  ppn=16

#optional: (frontera,levante,stampede2,etc.)
#ibatch     = 2              #0: serial mode;  1: parallel mode;  2: interactive/parallel jobs
...
if os.getenv('job_on_node')==None:
   ...
   if fmt==1 and iplot==1:
      while (not fexist('pread.done')): time.sleep(10)
      pcode=get_hpc_command('plot_icm.py',bdir,jname,qnode,nnode,ppn,walltime,scrout,fmt,'param',qname,account,reservation)
      os.system(pcode); os.remove('pread.done')
   os._exit(0)
...
#combine results
if ibatch in [1,2]: comm.Barrier()
if myrank==0:
   S=zdata(); S.bp=read(bpfile); S.time=[]; fnss=[]; nt=int(1e10)
   for i,[k,m] in enumerate(zip(svars,rvars)):
       ...
       if len(data)>0: S.attr(m,concatenate(data,axis=1)); mtime=concatenate(mtime); nt=int(min([nt,len(mtime)]))
   ...
   [S.attr(m,S.attr(m)[:,:nt]) for m in rvars if (S.hasattr(m) and nt!=S.attr(m).shape[1])]; S.time=S.time[:nt] #remove record
   ...
   if iplot==1: fid=open('pread.done','w+'); fid.close()
