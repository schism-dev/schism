...
!     Determination of redistribution is done using vsource.th.control, which is some statistic of flow for each source (e.g. 90th
!     percentile or mean)
...
!     Inputs: redistribute_source.in, constants below, hgrid.gr3, source_sink.in.0, vsource.th.0, vsource.th.control
...
!      read*, dt
      ...
      open(16,file='vsource.th.control',status='old')
...
!          xnd(i)=xcpp; ynd(i)=ycpp
     ...
     &(znd(n4)-znd(n1))*lframe(3,2)
      ...
      nline=0
      av_flow=0 !mean of control flow @ each source (can be single line file with statistic other than mean)
      do
        read(16,*,end=99)time,flow(1:nsource)
        av_flow=av_flow+flow
        nline=nline+1
      end do 
99    if(nline==0) stop 'vsource.th.control empty'
