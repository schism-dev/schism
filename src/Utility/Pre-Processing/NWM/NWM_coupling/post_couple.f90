...
!
!ifort -CB -g -traceback -o post_couple ../../../UtilLib/schism_geometry.f90 post_couple.f90 
...
!      read existing source/sink files
       ...
       allocate(vsource(nsource,nt),time(nt))
       open(15,file='vsource.th',status='old') 
       do i=1,nt
         read (15,*) time(i),vsource(:,i)
       enddo
       close(15)
...
!     write vsource.th file
      ...
      do i=1,nt
        write(16,'(10000F15.3)')time(i),(vsource(k,i),k=1,nsource)
      end do

!     write vsink.th
      open(17,file='vsink.th.1')
      do i=1,nt
        write(17,'(10000F15.3)')time(i),(vsink(k,i),k=1,nsink)
      end do
