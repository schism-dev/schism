...
!                     8th line: 1: use CBP casts to interpolate T/S inside ChesBay; 0: do not use CBP casts
!     (6) HYCOM files: [SSH,TS,UV]_[1,2,..nfiles].nc (beware scaling etc)
! 
!     if using CBP cast, 
!       matlab -nosplash -nodesktop -r "inputs_gen('hotstart.nc', [rundir])"
!     , which generates (7) and (8):
!     (7) CBP-cast for temperature and salinity extracted at the run's start time: [stName].tem.hot, [stName].sal.hot
!     (8) coordinates of CBP-casts in lon/lat: hot.in
...
!     First statement
      ...
      read(10,*) i_obs !# of stacks of HYCOM files
      close(10)
      if(tem_es<0.or.sal_es<0.or.tem_outside<0.or.sal_outside<0) &
     &stop 'Invalid T,S constants'
      if(i_obs<0.or.i_obs>2) stop 'Invalid CBP option'
...
!     Read in hgrid and vgrid
      ...
      do i=1,np
        ...
        iest(i)=nint(tmp)
        if(iest(i)/=0.and.iest(i)/=1.and.iest(i)/=2) then
...
!             if(z(i,k)<=-ht) saltout(k,i)=max(saltout(k,i),smin)
        ...
        if (i_obs==1) then !CB
          do iter=1,2  !1: salinity; 2: temperature
            iTest=0
            !Read CBP casts
            open(71,file='hot.in',status='old')
            read (71,*) nSt, nSt_J
            do k=1,nSt+nSt_J
                read (71,*) x0(k), y0(k), stName
                if (iter==1) then
                    open(72,file=trim(adjustl(stName))//'.sal.hot',status='old')
                else
                    open(72,file=trim(adjustl(stName))//'.tem.hot',status='old')
                endif
                read (72,*) nDepth(k)
                print *, "station: ", k, stName
                do m=1,nDepth(k)
            ...
            !inside bay, interpolate from CBP casts, FY
            do i=1,np
              if(dp(i)<=0) then
                  temp2(:)=0; !stop 'Dry'
                  cycle
              ...
              if(iest(i)==0) then !outside Bay
                cycle
              elseif(iest(i)==1) then  !main stem
                  if (yl(i)>=y0(1)) then
                      nn(1)=1; nn(2)=1
          ...
          do i=1,np
            ...
            if (iest(i)==2) then !in DB
              ...
              enddo
              if (i==4711.or.i==331640) then
                print *,'eout in DB at: ',i,eout(i)
