...
def relocate_sources2(
    ...
    :no_feeder: should be set to False in normal cases, in which sources on old_ss_dir/hgrid.gr3
        will be relocated to the feeder heads of run_dir/hgrid.gr3.
        In special cases where run_dir/hgrid.gr3 is the same as old_ss_dir/hgrid.gr3 and
        both are without feeders, set no_feeder to true, so that the locations of feeder_bases
        will be used for relocation instead of feeder_heads. The feeder info should also be
        generated by make_feeder_channel.py even though the feeders are not implemented in the mesh.
        In this case, the relocation still moves sources to resolved channels.
