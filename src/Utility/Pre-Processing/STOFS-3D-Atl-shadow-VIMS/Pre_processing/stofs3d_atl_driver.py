...
def main():
    ...
    hgrid_path = '/sciclone/schism10/feiye/STOFS3D-v8/I15d4_v7/Bathy_edit/Temporary_Fix_v7.2/temporary_fix_river.gr3'
    ...
    scr_dir = '/sciclone/scr10/feiye/'  # None if the outputs are saved in the run directory
    ...
    # Normal case:
    # Need a hgrid without feeders to generate the original sources/sinks.
    # This is necessary because the no-feeder hgrid gives the correct source locations,
    # and the feeder channels are not real channels.
    # moreover, the land boundary is set to minimize NWM segments weaving in and out.

    # Special case:
    # When a hgrid does not have feeders, the option "no_feeder" should be set to False

    if input_files['source_sink']:
        sub_dir = 'Source_sink'
        print(f'{DRIVER_PRINT_PREFIX}Generating source_sink.in ...')

        if config.no_feeder_hgrid is not None:
            print(f'An hgrid without feeders is provided: {config.no_feeder_hgrid}',
                  'assuming the main hgrid has feeders')
            main_hgrid_no_feeder = False
        else:
            print('An hgrid without feeders is not specified,'
                  'assuming the main hgrid has NO feeders!')
            main_hgrid_no_feeder = True
        ...
        if config.no_feeder_hgrid is not None:
            os.symlink(f'{config.no_feeder_hgrid}', 'hgrid.gr3')
        else:
        ...
        if config.relocate_source:
            # '''
            # # save a copy of the original source_sink files
            ...
            relocate_sources2(
                ...
                allow_neglection=False, no_feeder=main_hgrid_no_feeder
            )
            # regenerate vsource.th based on relocated sources.json
            # No need for sinks.json because they are discarded.
            # Don't call gen_sourcesink_nwm again, which is time consuming.
            # Also, some sources may be adjusted by USGS obs before this step
            ...
            # Consider retaining both options.
            # '''
            if False:
