...
from pylib import read_schism_hgrid, inside_polygon, read_schism_bpfile, grd2sms, sms2grd
...
def convert2xgeoid(wdir, hgrid_obj, diag_output=None):
    ...
    fname = 'hgrid_stofs3d_inland_ches_del.txt'
    ...
    # Try region 5 first.
    if fname in generated_input_files:
        input_fnames.append(fname)
        processes.append(subprocess.Popen(
            f"java -jar vdatum.jar ihorz:NAD83_2011 ivert:navd88:m:{z_convention} "
            f"ohorz:igs14:geo:deg overt:xgeoid20b:m:{z_convention} "
            f"-file:txt:space,1,2,3,skip0:{fname}:result region:5",
            shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL))
    ...
    for i, process in enumerate(processes):
        ...
        if exit_code == 0:
            ...
            raise ValueError(f"Process {i+1} ({input_fnames[i]}) failed with exit code {exit_code}")
...
def get_depth_diff():
    """
    Get the difference between the original depth and the converted depth
    """
    hg1 = cread_schism_hgrid('/sciclone/schism10/feiye/STOFS3D-v8/I20/Bathy_edit3/hgrid.ll')
    hg2 = sms2grd('/sciclone/schism10/feiye/STOFS3D-v8/I20/Bathy_edit/Levee/hgrid_ll_dem_NCF_levee.2dm')
    hg1.write_hgrid(
        '/sciclone/schism10/feiye/STOFS3D-v8/I20/Bathy_edit/xGEOID/hgrid_xGEOID20b_dp_minus_NAVD_dp.gr3',
        value=hg1.dp-hg2.dp, fmt=1
    )

    print('Done!')


def convert_based_on_existing_diff():
    """
    Convert the depths based on the existing depth difference file, e.g.:
    hgrid_xGEOID20b_dp_minus_NAVD_dp.gr3,
    which is generated by convert2xgeoid()
    """
    hg = cread_schism_hgrid('/sciclone/schism10/feiye/STOFS3D-v8/I20/Bathy_edit3/hgrid.ll')
    hg_depth_diff = read_schism_hgrid('/sciclone/schism10/feiye/STOFS3D-v8/I20/Bathy_edit/xGEOID/hgrid_xGEOID20b_dp_minus_NAVD_dp.gr3')
    hg.dp = hg.dp + hg_depth_diff.dp
    hg.write_hgrid(f'/sciclone/schism10/feiye/STOFS3D-v8/I20/Bathy_edit3/xGEOID/hgrid_diffed_to_xgeoid20b.gr3', fmt=1)

    print('Done!')


if __name__ == "__main__":
    convert_based_on_existing_diff()
