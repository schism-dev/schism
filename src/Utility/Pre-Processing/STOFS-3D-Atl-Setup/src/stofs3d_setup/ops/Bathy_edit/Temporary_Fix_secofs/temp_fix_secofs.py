...
def temp_fix_secofs(gd_ll, wdir, reference_hgrid_file):
    ...
    secofs_gdf = gpd.read_file(
        f'{wdir}/secofs_domain_clipped_v1.shp'
    )
    ...
    if gd_ll.np != reference_hgrid.np:
        raise ValueError(
            'Target and reference hgrids have different numbers of nodes: '
            f'{gd_ll.np} vs {reference_hgrid.np}.'
        )
