/* $Id: drawticks.c,v 1.2 2003/07/24 15:23:44 pturner Exp $
 *
...
void drawyaxisbar(int gno, int caxis)
{
    ...
    double start, stop;
    int xinvflag = isxreversed(gno) ? -1 : 1;
...
void create_ticklabel(int form, int prec, double loc, char *s)
{
	...
	break;
    case DDMONTHSYY:
	strcpy(format, "%02d-%s-%d");
	calcdate(loc, &m, &d, &y, &h, &mm, &sec);
	if (y >= 1900 && y < 2000) {
	    y -= 1900;
	}
	sprintf(s, format, d, months[m - 1], y);
	break;
    case DDMONTHSYYHHMMSS:
	strcpy(format, "%02d-%s-%d %02d:%02d:%02d");
	calcdate(loc, &m, &d, &y, &h, &mm, &sec);
	sprintf(s, format, y, months[m - 1], d, h, mm, (int) sec);
	break;
    case MONTHL:
	strcpy(format, "%s");
	calcdate(loc, &m, &d, &y, &h, &mm, &sec);
...
void drawxticklabels(int gno, int caxis)
{
    ...
    double delb = 0.0, delt = 0.0, vyb, vyt, vx, ofx, ofb, oft;
    double start, stop;
    double dx, dt, ttmp;	/* DEFUNCT for alternate mapping */
...
/* draw tick labels */
	...
	} else if (t.tl_format == DDMONTHSYYHHMMSS) {
	        create_ticklabel(DDMONTHSYY, t.tl_prec, loc, s);
	        iy = ifudge;
	        (*devwritestr) ((*devconvx) (x), (*devconvy) (bot - delb) - iy * (1 + i % (t.tl_staggered + 1)), tang, s, bjust, 1);
	        create_ticklabel(HMS, t.tl_prec, loc, s);
	        iy = 2 * ifudge;
	        (*devwritestr) ((*devconvx) (x), (*devconvy) (bot - delb) - iy * (1 + i % (t.tl_staggered + 1)), tang, s, bjust, 1);
		continue;
	} else {
...
void drawyticklabels(int gno, int caxis)
{
    ...
    double delr = 0.0, dell = 0.0, vxl, vxr, vy, ofy, ofr, ofl;
    double start, stop;
    double dy, dt, ttmp;	/* DEFUNCT for alternate mapping */
...
void drawxtickmarks(int gno, int caxis)
{
    ...
    double delb = 0.0, delt = 0.0, vyb, vyt, vx, ofx, ofb, oft;
    double dx, dt, ttmp1, ttmp2;/* DEFUNCT for alternate mapping */
	...
	if (((t.t_op == BOTTOM && !yinvflag) || ((t.t_op == TOP) && yinvflag)) 
		...
		if (logloop != 0) {
			...
			if (t.t_gridflag == ON && i == 0) {
				...
				break;
			    case BOTH:	/* draw up from y1 then draw down
					 * from y2 */
				drawtic(start, bot - delb, axis, yinvflag ? 1 : 0);
				drawtic(start, bot - delb, axis, yinvflag ? 0 : 1);
				break;
