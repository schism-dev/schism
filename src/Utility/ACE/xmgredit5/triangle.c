...
/*
 * PJT added to re-direct messages to a string
 */
#include <stdarg.h>
...
#ifdef CDT_ONLY
#ifdef REDUCED
  my_printf("triangle [-pAcevngBPNEIOXzo_lQVh] input_file\n");
#else /* not REDUCED */
  my_printf("triangle [-pAcevngBPNEIOXzo_iFlCQVh] input_file\n");
#endif /* not REDUCED */
#else /* not CDT_ONLY */
#ifdef REDUCED
  my_printf("triangle [-prq__a__AcevngBPNEIOXzo_YS__lQVh] input_file\n");
#else /* not REDUCED */
  my_printf("triangle [-prq__a__AcevngBPNEIOXzo_YS__iFlsCQVh] input_file\n");
#endif /* not REDUCED */
#endif /* not CDT_ONLY */

  my_printf("    -p  Triangulates a Planar Straight Line Graph (.poly file).\n");
#ifndef CDT_ONLY
  my_printf("    -r  Refines a previously generated mesh.\n");
  my_printf(
    "    -q  Quality mesh generation.  A minimum angle may be specified.\n");
  my_printf("    -a  Applies a maximum triangle area constraint.\n");
#endif /* not CDT_ONLY */
  my_printf(
    "    -A  Applies attributes to identify elements in certain regions.\n");
  my_printf("    -c  Encloses the convex hull with segments.\n");
  my_printf("    -e  Generates an edge list.\n");
  my_printf("    -v  Generates a Voronoi diagram.\n");
  my_printf("    -n  Generates a list of triangle neighbors.\n");
  my_printf("    -g  Generates an .off file for Geomview.\n");
  my_printf("    -B  Suppresses output of boundary information.\n");
  my_printf("    -P  Suppresses output of .poly file.\n");
  my_printf("    -N  Suppresses output of .node file.\n");
  my_printf("    -E  Suppresses output of .ele file.\n");
  my_printf("    -I  Suppresses mesh iteration numbers.\n");
  my_printf("    -O  Ignores holes in .poly file.\n");
  my_printf("    -X  Suppresses use of exact arithmetic.\n");
  my_printf("    -z  Numbers all items starting from zero (rather than one).\n");
  my_printf("    -o2 Generates second-order subparametric elements.\n");
#ifndef CDT_ONLY
  my_printf("    -Y  Suppresses boundary segment splitting.\n");
  my_printf("    -S  Specifies maximum number of added Steiner points.\n");
#endif /* not CDT_ONLY */
#ifndef REDUCED
  my_printf("    -i  Uses incremental method, rather than divide-and-conquer.\n");
  my_printf("    -F  Uses Fortune's sweepline algorithm, rather than d-and-c.\n");
#endif /* not REDUCED */
  my_printf("    -l  Uses vertical cuts only, rather than alternating cuts.\n");
#ifndef REDUCED
#ifndef CDT_ONLY
  my_printf(
    "    -s  Force segments into mesh by splitting (instead of using CDT).\n");
#endif /* not CDT_ONLY */
  my_printf("    -C  Check consistency of final mesh.\n");
#endif /* not REDUCED */
  my_printf("    -Q  Quiet:  No terminal output except errors.\n");
  my_printf("    -V  Verbose:  Detailed information on what I'm doing.\n");
  my_printf("    -h  Help:  Detailed instructions for Triangle.\n");
  exit(0);
}
...
void internalerror()
{
  my_printf("  Please report this bug to jrs@cs.cmu.edu\n");
  my_printf("  Include the message above, your input data set, and the exact\n");
  my_printf("    command line you used to run Triangle.\n");
  exit(1);
}
      ...
      for (j = STARTINDEX; argv[i][j] != '\0'; j++) {
        ...
        if (argv[i][j] == 'a') {
          ...
          if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
            ...
            if (maxarea <= 0.0) {
              my_printf("Error:  Maximum area must be greater than zero.\n");
              exit(1);
	    }
	  } else {
            vararea = 1;
...
struct triedge *t;
{
  ...
  my_printf("triangle x%lx with orientation %d:\n", (unsigned long) t->tri,
         t->orient);
  decode(t->tri[0], printtri);
  if (printtri.tri == dummytri) {
    my_printf("    [0] = Outer space\n");
  } else {
  ...
  if (printpoint == (point) NULL)
    my_printf("    Origin[%d] = NULL\n", (t->orient + 1) % 3 + 3);
  else
    my_printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 1) % 3 + 3, (unsigned long) printpoint,
           printpoint[0], printpoint[1]);
  dest(*t, printpoint);
  if (printpoint == (point) NULL)
    my_printf("    Dest  [%d] = NULL\n", (t->orient + 2) % 3 + 3);
  else
    my_printf("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 2) % 3 + 3, (unsigned long) printpoint,
           printpoint[0], printpoint[1]);
  apex(*t, printpoint);
  if (printpoint == (point) NULL)
    my_printf("    Apex  [%d] = NULL\n", t->orient + 3);
  else
    my_printf("    Apex  [%d] = x%lx  (%.12g, %.12g)\n",
...
struct edge *s;
{
  ...
  my_printf("shell edge x%lx with orientation %d and mark %d:\n",
         (unsigned long) s->sh, s->shorient, mark(*s));
  sdecode(s->sh[0], printsh);
  if (printsh.sh == dummysh) {
    my_printf("    [0] = No shell\n");
  } else {
    my_printf("    [0] = x%lx  %d\n", (unsigned long) printsh.sh,
           printsh.shorient);
  }
...
void exactinit()
{
  ...
  splitter += 1.0;
  if (verbose > 1) {
    my_printf("Floating point roundoff is of magnitude %.17g\n", epsilon);
    my_printf("Floating point splitter is %.17g\n", splitter);
  }
...
void checkmesh()
{
  ...
  noexact = 0;
  if (!quiet) {
    my_printf("  Checking consistency of mesh...\n");
  }
  horrors = 0;
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      if (oppotri.tri != dummytri) {
        ...
        if ((triangleloop.tri != oppooppotri.tri)
          ...
          my_printf("  !! !! Asymmetric triangle-triangle bond:\n");
          if (triangleloop.tri == oppooppotri.tri) {
            my_printf("   (Right triangle, wrong orientation)\n");
          }
          my_printf("    First ");
          printtriangle(&triangleloop);
          my_printf("    Second (nonreciprocating) ");
        ...
        if ((triorg != oppodest) || (tridest != oppoorg)) {
          my_printf("  !! !! Mismatched edge coordinates between two triangles:\n"
                 );
          my_printf("    First mismatched ");
          printtriangle(&triangleloop);
          my_printf("    Second mismatched ");
          printtriangle(&oppotri);
          horrors++;
        }
      }
  ...
  if (horrors == 0) {
    if (!quiet) {
      my_printf("  In my studied opinion, the mesh appears to be consistent.\n");
    }
  } else if (horrors == 1) {
    my_printf("  !! !! !! !! Precisely one festering wound discovered.\n");
  } else {
    my_printf("  !! !! !! !! %d abominations witnessed.\n", horrors);
  }
...
void checkdelaunay()
{
  ...
  noexact = 0;
  if (!quiet) {
    my_printf("  Checking Delaunay property of mesh...\n");
  }
  horrors = 0;
  /* Run through the list of triangles, checking each one. */
  traversalinit(&triangles);
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      if (shouldbedelaunay) {
        if (incircle(triorg, tridest, triapex, oppoapex) > 0.0) {
          my_printf("  !! !! Non-Delaunay pair of triangles:\n");
          my_printf("    First non-Delaunay ");
          printtriangle(&triangleloop);
          my_printf("    Second non-Delaunay ");
          printtriangle(&oppotri);
          horrors++;
        }
      }
  ...
  "  By virtue of my perceptive intelligence, I declare the mesh Delaunay.\n");
    ...
    my_printf("  !! !! !! !! %d obscenities viewed with horror.\n", horrors);
  }
  /* Restore the status of exact arithmetic. */
  noexact = saveexact;
}
...
point insdest;
{
  ...
  if (verbose > 2) {
    my_printf("  Queueing bad triangle:\n");
    my_printf("    (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n", insorg[0],
...
struct edge *testedge;
{
  ...
  if (addtolist && (!nobisect || ((nobisect == 1) && (sides == 2)))) {
    if (verbose > 2) {
      my_printf("  Queueing encroached segment (%.12g, %.12g) (%.12g, %.12g).\n",
...
void makepointmap()
{
  ...
  if (verbose) {
    my_printf("    Constructing mapping from points to triangles.\n");
  }
  traversalinit(&triangles);
  triangleloop.tri = triangletraverse();
  while (triangleloop.tri != (triangle *) NULL) {
...
struct triedge *searchtri;
{
  ...
  if (verbose > 2) {
    my_printf("  Searching for point (%.12g, %.12g).\n",
  ...
  while (1) {
    if (verbose > 2) {
      my_printf("    At (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n",
...
struct triedge *searchtri;
{
  ...
  if (verbose > 2) {
    my_printf("  Randomly sampling for a triangle near point (%.12g, %.12g).\n",
           searchpoint[0], searchpoint[1]);
  }
  ...
  if (verbose > 2) {
    my_printf("    Boundary triangle has origin (%.12g, %.12g).\n",
           torg[0], torg[1]);
  }
  ...
  if (recenttri.tri != (triangle *) NULL) {
    if (recenttri.tri[3] != (triangle) NULL) {
      ...
      if (dist < searchdist) {
        ...
        searchdist = dist;
        if (verbose > 2) {
          my_printf("    Choosing recent triangle with origin (%.12g, %.12g).\n",
                 torg[0], torg[1]);
        }
      }
    }
    ...
    for (j = 0; j < samplesperblock; j++) {
      ...
      if (sampletri.tri[3] != (triangle) NULL) {
        ...
        if (dist < searchdist) {
          ...
          searchdist = dist;
          if (verbose > 2) {
            my_printf("    Choosing triangle with origin (%.12g, %.12g).\n",
                   torg[0], torg[1]);
          }
        }
      }
...
struct triedge *flipedge;                    /* Handle for the triangle abc. */
{
...
#ifdef SELF_CHECK
  if (top.tri == dummytri) {
    my_printf("Internal error in flip():  Attempt to flip on boundary.\n");
    lnextself(*flipedge);
    return;
  }
  if (checksegments) {
    ...
    if (toplshelle.sh != dummysh) {
      my_printf("Internal error in flip():  Attempt to flip a segment.\n");
      lnextself(*flipedge);
      return;
    }
  }
  ...
  if (verbose > 2) {
    my_printf("  Edge flip results in left ");
    lnextself(topleft);
...
int triflaws;
{
  ...
  if ((intersect == ONEDGE) || (intersect == OUTSIDE)) {
    ...
    if (counterclockwise(rightpoint, leftpoint, botpoint) < 0.0) {
      my_printf("Internal error in insertsite():\n");
      my_printf("  Clockwise triangle prior to edge point insertion (bottom).\n");
    }
    if (mirrorflag) {
      if (counterclockwise(leftpoint, rightpoint, toppoint) < 0.0) {
        my_printf("Internal error in insertsite():\n");
        my_printf("  Clockwise triangle prior to edge point insertion (top).\n");
      }
      if (counterclockwise(rightpoint, toppoint, insertpoint) < 0.0) {
        my_printf("Internal error in insertsite():\n");
        my_printf("  Clockwise triangle after edge point insertion (top right).\n"
               );
      }
      if (counterclockwise(toppoint, leftpoint, insertpoint) < 0.0) {
        my_printf("Internal error in insertsite():\n");
        my_printf("  Clockwise triangle after edge point insertion (top left).\n"
               );
      }
    }
    if (counterclockwise(leftpoint, botpoint, insertpoint) < 0.0) {
      my_printf("Internal error in insertsite():\n");
      my_printf("  Clockwise triangle after edge point insertion (bottom left).\n"
             );
    }
...
#endif /* SELF_CHECK */
    if (verbose > 2) {
      my_printf("  Updating bottom left ");
      printtriangle(&botright);
      if (mirrorflag) {
        my_printf("  Updating top left ");
        printtriangle(&topright);
        my_printf("  Creating top right ");
        printtriangle(&newtopright);
      }
      my_printf("  Creating bottom right ");
    ...
    if (counterclockwise(rightpoint, leftpoint, botpoint) < 0.0) {
      my_printf("Internal error in insertsite():\n");
      my_printf("  Clockwise triangle prior to point insertion.\n");
    }
    if (counterclockwise(rightpoint, leftpoint, insertpoint) < 0.0) {
      my_printf("Internal error in insertsite():\n");
      my_printf("  Clockwise triangle after point insertion (top).\n");
    }
    if (counterclockwise(leftpoint, botpoint, insertpoint) < 0.0) {
      my_printf("Internal error in insertsite():\n");
      my_printf("  Clockwise triangle after point insertion (left).\n");
    }
    if (counterclockwise(botpoint, rightpoint, insertpoint) < 0.0) {
      my_printf("Internal error in insertsite():\n");
      my_printf("  Clockwise triangle after point insertion (right).\n");
    }
#endif /* SELF_CHECK */
    if (verbose > 2) {
      my_printf("  Updating top ");
      printtriangle(&horiz);
      my_printf("  Creating left ");
      printtriangle(&newbotleft);
      my_printf("  Creating right ");
      printtriangle(&newbotright);
    }
  }
        ...
        if (doflip) {
          ...
          if (insertpoint != (point) NULL) {
            if (counterclockwise(leftpoint, insertpoint, rightpoint) < 0.0) {
              my_printf("Internal error in insertsite():\n");
              my_printf("  Clockwise triangle prior to edge flip (bottom).\n");
            }
...
/*
            if (counterclockwise(rightpoint, farpoint, leftpoint) < 0.0) {
              my_printf("Internal error in insertsite():\n");
              my_printf("  Clockwise triangle prior to edge flip (top).\n");
            }
*/
            if (counterclockwise(farpoint, leftpoint, insertpoint) < 0.0) {
              my_printf("Internal error in insertsite():\n");
              my_printf("  Clockwise triangle after edge flip (left).\n");
            }
            if (counterclockwise(insertpoint, rightpoint, farpoint) < 0.0) {
              my_printf("Internal error in insertsite():\n");
              my_printf("  Clockwise triangle after edge flip (right).\n");
            }
          }
#endif /* SELF_CHECK */
          if (verbose > 2) {
...
int triflaws;
{
  ...
  if (verbose > 2) {
    my_printf("  Triangulating interior polygon at edge\n");
    my_printf("    (%.12g, %.12g) (%.12g, %.12g)\n", leftbasepoint[0],
  ...
  if (verbose > 2) {
    my_printf("    Connecting edge to (%.12g, %.12g)\n", bestpoint[0],
           bestpoint[1]);
  }
  if (bestnumber > 1) {
...
struct triedge *deltri;
{
  ...
  while (!triedgeequal(*deltri, countingtri)) {
    ...
    if (countingtri.tri == dummytri) {
      my_printf("Internal error in deletesite():\n");
      my_printf("  Attempt to delete boundary point.\n");
      internalerror();
    }
#endif /* SELF_CHECK */
    edgecount++;
...
int axis;
{
  ...
  if (verbose > 2) {
    my_printf("  Creating base bounding ");
  ...
  while (1) {
    ...
    if (leftfinished && rightfinished) {
      ...
      if (verbose > 2) {
        my_printf("  Creating top bounding ");
        printtriangle(&baseedge);
      }
      /* Special treatment for horizontal cuts. */
      if (dwyer && (axis == 1)) {
...
struct triedge *farright;
{
  ...
  if (verbose > 2) {
    my_printf("  Triangulating %d points.\n", vertices);
  }
  if (vertices == 2) {
    ...
    divconqrecurse(&sortarray[divider], vertices - divider, 1 - axis,
                   &innerright, farright);
    if (verbose > 1) {
      my_printf("  Joining triangulations with %d and %d vertices.\n", divider,
             vertices - divider);
    }
...
struct triedge *startghost;
{
  ...
  if (verbose) {
    my_printf("  Removing ghost triangles.\n");
  }
...
*/
  ...
  if (verbose) {
    my_printf("  Forming triangulation.\n");
  }
...
void boundingbox()
{
  ...
  if (verbose) {
    my_printf("  Creating triangular bounding box.\n");
  }
...
long removebox()
{
  ...
  if (verbose) {
    my_printf("  Removing triangular bounding box.\n");
  }
  /* Find a boundary triangle. */
...
long incrementaldelaunay()
{
  ...
  boundingbox();
  if (verbose) {
    my_printf("  Incrementally inserting points.\n");
  }
  traversalinit(&points);
...
long sweeplinedelaunay()
{
  ...
  if (verbose) {
    my_printf("  Placing points in event heap.\n");
  }
  ...
  do {
    if (heapsize == 0) {
      my_printf("Error:  Input points are all identical.\n");
      exit(1);
    }
...
#else /* not REDUCED */
  if (!quiet) {
    my_printf("Constructing Delaunay triangulation ");
    if (incremental) {
      my_printf("by incremental method.\n");
    } else if (sweepline) {
      my_printf("by sweepline method.\n");
    } else {
      my_printf("by divide-and-conquer method.\n");
    }
  }
  if (incremental) {
...

{
  ...
  incorners = corners;
  if (incorners < 3) {
    my_printf("Error:  Triangles must have at least 3 points.\n");
    exit(1);
  }
  eextras = attribs;
#else /* not TRILIBRARY */
  ...
  if (elefile == (FILE *) NULL) {
    my_printf("  Error:  Cannot access file %s.\n", elefilename);
    exit(1);
  }
  ...
  if (*stringptr == '\0') {
    ...
    if (incorners < 3) {
      my_printf("Error:  Triangles in %s must have at least 3 points.\n",
             elefilename);
      exit(1);
    }
  }
  ...
  if (vararea) {
    ...
    if (areaelements != inelements) {
      my_printf("Error:  %s and %s disagree on number of triangles.\n",
  ...
  if (verbose) {
    my_printf("  Assembling triangles.\n");
  }
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      if ((corner[j] < firstnumber) || (corner[j] >= firstnumber + inpoints)) {
        my_printf("Error:  Triangle %d has an invalid vertex index.\n",
               elementnumber);
        exit(1);
      }
    }
    ...
    for (j = 0; j < 3; j++) {
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        my_printf("Error:  Triangle %d is missing point %d in %s.\n",
  ...
  if (poly) {
    if (verbose) {
      my_printf("  Marking segments in triangulation.\n");
    }
    ...
    while (shelleloop.sh != (shelle *) NULL) {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        my_printf("Error:  Segment %d has no endpoints in %s.\n", segmentnumber,
               polyfilename);
        exit(1);
      } else {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        my_printf("Error:  Segment %d is missing its second endpoint in %s.\n",
      ...
      for (j = 0; j < 2; j++) {
        if ((end[j] < firstnumber) || (end[j] >= firstnumber + inpoints)) {
          my_printf("Error:  Segment %d has an invalid vertex index.\n", 
                 segmentnumber);
          exit(1);
        }
      }
...
point endpoint;
{
  ...
  while (leftflag) {
    ...
    onextself(*searchtri);
    if (searchtri->tri == dummytri) {
      my_printf("Internal error in finddirection():  Unable to find a\n");
      my_printf("  triangle leading from (%.12g, %.12g) to", startpoint[0],
             startpoint[1]);
...
point endpoint2;
{
  ...
  denom = ty * ex - tx * ey;
  if (denom == 0.0) {
    my_printf("Internal error in segmentintersection():");
    my_printf("  Attempt to find intersection of parallel segments.\n");
    internalerror();
  }
  ...
  if (success != SUCCESSFULPOINT) {
    my_printf("Internal error in segmentintersection():\n");
    my_printf("  Failure to split a segment.\n");
    internalerror();
  }
  if (steinerleft > 0) {
    steinerleft--;
  ...
  if ((leftpoint[0] == endpoint1[0]) && (leftpoint[1] == endpoint1[1])) {
    ...
    my_printf("Internal error in segmentintersection():\n");
    my_printf("  Topological inconsistency after splitting a segment.\n");
    internalerror();
  }
...
int newmark;
{
  ...
  if (verbose > 2) {
    my_printf("Forcing segment into triangulation by recursive splitting:\n");
  ...
  if (success == DUPLICATEPOINT) {
    if (verbose > 2) {
      my_printf("  Segment intersects existing point (%.12g, %.12g).\n",
    ...
    if (success == VIOLATINGPOINT) {
      if (verbose > 2) {
        my_printf("  Two segments intersect at (%.12g, %.12g).\n",
               newpoint[0], newpoint[1]);
      }
      ...
      if (success != SUCCESSFULPOINT) {
        my_printf("Internal error in conformingedge():\n");
        my_printf("  Failure to split a segment.\n");
        internalerror();
      }
    }
...
int newmark;
{
  ...
  if (verbose > 1) {
    my_printf("  Connecting (%.12g, %.12g) to (%.12g, %.12g).\n",
  ...
  if (checkpoint != endpoint1) {
    ...
    if (locate(endpoint1, &searchtri1) != ONVERTEX) {
      ...
      my_printf("  (%.12g, %.12g) in triangulation.\n",
             endpoint1[0], endpoint1[1]);
      internalerror();
    }
  }
...

{
  ...
  if (poly) {
    if (!quiet) {
      my_printf("Inserting segments into Delaunay triangulation.\n");
    }
#ifdef TRILIBRARY
    ...
    if (segments > 0) {
      if (verbose) {
        my_printf("  Inserting PSLG segments.\n");
      }
      makepointmap();
    }
    ...
    for (i = 1; i <= segments; i++) {
      ...
      if ((end1 < firstnumber) || (end1 >= firstnumber + inpoints)) {
        if (!quiet) {
          my_printf("Warning:  Invalid first endpoint of segment %d in %s.\n", i,
                 polyfilename);
        }
      } else if ((end2 < firstnumber) || (end2 >= firstnumber + inpoints)) {
        if (!quiet) {
          my_printf("Warning:  Invalid second endpoint of segment %d in %s.\n", i,
                 polyfilename);
        }
      } else {
        ...
        if ((endpoint1[0] == endpoint2[0]) && (endpoint1[1] == endpoint2[1])) {
          if (!quiet) {
            my_printf("Warning:  Endpoints of segment %d are coincident in %s.\n",
                   i, polyfilename);
          }
        } else {
  ...
  if (convex || !poly) {
    ...
    if (verbose) {
      my_printf("  Enclosing convex hull with segments.\n");
    }
    markhull();
  }
  return segments;
...
void infecthull()
{
  ...
  if (verbose) {
    my_printf("  Marking concavities (external triangles) for elimination.\n");
  }
  /* Find a triangle handle on the hull. */
  hulltri.tri = dummytri;
  hulltri.orient = 0;
...
void plague()
{
  ...
  if (verbose) {
    my_printf("  Marking neighbors of marked triangles.\n");
  }
  ...
  while (virusloop != (triangle **) NULL) {
    ...
    if (verbose > 2) {
      ...
      my_printf("    Checking (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n",
  ...
  if (verbose) {
    my_printf("  Deleting marked triangles.\n");
  }
  traversalinit(&viri);
  virusloop = (triangle **) traverse(&viri);
  while (virusloop != (triangle **) NULL) {
    ...
    for (testtri.orient = 0; testtri.orient < 3; testtri.orient++) {
      ...
      if (testpoint != (point) NULL) {
        ...
        if (killorg) {
          if (verbose > 1) {
            my_printf("    Deleting point (%.12g, %.12g)\n",
...
REAL area;
{
    ...
    for (testtri.orient = 0; testtri.orient < 3; testtri.orient++) {
      ...
      if ((neighbor.tri != dummytri) && !infected(neighbor)
        ...
        if (verbose > 2) {
          ...
          my_printf("    Marking (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n",
                 regionorg[0], regionorg[1], regiondest[0], regiondest[1],
                 regionapex[0], regionapex[1]);
        }
        /* Infect the neighbor. */
  ...
  if (verbose > 1) {
    my_printf("  Unmarking marked triangles.\n");
  }
  traversalinit(&viri);
  virusloop = (triangle **) traverse(&viri);
  while (virusloop != (triangle **) NULL) {
...
int regions;
{
  ...
  if (!(quiet || (noholes && convex))) {
    my_printf("Removing unwanted triangles.\n");
    if (verbose && (holes > 0)) {
      my_printf("  Marking holes for elimination.\n");
    }
  }
  ...
  if (regions > 0) {
    if (!quiet) {
      if (regionattrib) {
        if (vararea) {
          my_printf("Spreading regional attributes and area constraints.\n");
        } else {
          my_printf("Spreading regional attributes.\n");
        }
      } else { 
        my_printf("Spreading regional area constraints.\n");
      }
    }
...
void precisionerror()
{
  my_printf("Try increasing the area criterion and/or reducing the minimum\n");
  my_printf("  allowable angle so that tiny triangles are not created.\n");
#ifdef SINGLE
  my_printf("Alternatively, try recompiling me with double precision\n");
  my_printf("  arithmetic (by removing \"#define SINGLE\" from the\n");
  my_printf("  source file or \"-DSINGLE\" from the makefile).\n");
#endif /* SINGLE */
}
  ...
  while ((badsegments.items > 0) && (steinerleft != 0)) {
    ...
    while ((encloop != (struct edge *) NULL) && (steinerleft != 0)) {
      ...
      if (((newpoint[0] == eorg[0]) && (newpoint[1] == eorg[1]))
        ...
        my_printf("Error:  Ran out of precision at (%.12g, %.12g).\n",
               newpoint[0], newpoint[1]);
        my_printf("I attempted to split a segment to a smaller size than can\n");
        my_printf("  be accommodated by the finite precision of floating point\n"
               );
        my_printf("  arithmetic.\n");
        precisionerror();
        exit(1);
      }
      ...
      if ((success != SUCCESSFULPOINT) && (success != ENCROACHINGPOINT)) {
        my_printf("Internal error in repairencs():\n");
        my_printf("  Failure to split a segment.\n");
        internalerror();
      }
      if (steinerleft > 0) {
...
void tallyfaces()
{
  ...
  if (verbose) {
    my_printf("  Making a list of bad triangles.\n");
  }
  traversalinit(&triangles);
  triangleloop.orient = 0;
  triangleloop.tri = triangletraverse();
...
struct badface *badtri;
{
  ...
  if ((borg == badtri->faceorg) && (bdest == badtri->facedest) &&
    ...
    if (verbose > 1) {
      my_printf("  Splitting this triangle at its circumcenter:\n");
      my_printf("    (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n", borg[0],
    ...
    if (((newpoint[0] == borg[0]) && (newpoint[1] == borg[1]))
      ...
      if (!quiet) {
        my_printf("Warning:  New point (%.12g, %.12g) falls on existing vertex.\n"
               , newpoint[0], newpoint[1]);
        errorflag = 1;
      }
      pointdealloc(newpoint);
    ...
    if (errorflag) {
      if (verbose) {
        my_printf("  The new point is at the circumcenter of triangle\n");
        my_printf("    (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n",
               borg[0], borg[1], bdest[0], bdest[1], bapex[0], bapex[1]);
      }
      my_printf("This probably means that I am trying to refine triangles\n");
      my_printf("  to a smaller size than can be accommodated by the finite\n");
      my_printf("  precision of floating point arithmetic.  (You can be\n");
      my_printf("  sure of this if I fail to terminate.)\n");
      precisionerror();
    }
  }
...
void enforcequality()
{
  ...
  if ((minangle > 0.0) || vararea || fixedarea) {
    ...
    tallyfaces();
    if (verbose) {
      my_printf("  Splitting bad triangles.\n");
    }
  ...
  if (!quiet && (badsegments.items > 0) && (steinerleft == 0)) {
    ...
    if (badsegments.items == 1) {
      my_printf("  an encroached segment, and therefore might not be truly\n");
    } else {
      my_printf("  %ld encroached segments, and therefore might not be truly\n",
             badsegments.items);
    }
    my_printf("  Delaunay.  If the Delaunay property is important to you,\n");
    my_printf("  try increasing the number of Steiner points (controlled by\n");
    my_printf("  the -S switch) slightly and try again.\n\n");
  }
}
...
void highorder()
{
  ...
  if (!quiet) {
    my_printf("Adding vertices for second-order triangles.\n");
  }
...
char *infilename;
{
  ...
  do {
    ...
    if (result == (char *) NULL) {
      my_printf("  Error:  Unexpected end of file in %s.\n", infilename);
      exit(1);
    }
...
FILE **polyfile;
{
  ...
  if (inpoints < 3) {
    my_printf("Error:  Input must have at least three input points.\n");
    exit(1);
  }
  if (mesh_dim != 2) {
    my_printf("Error:  Triangle only works with two-dimensional meshes.\n");
    exit(1);
  }
  ...
  for (i = 0; i < inpoints; i++) {
    ...
    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      my_printf("Error:  Point %d has no x coordinate.\n", firstnumber + i);
      exit(1);
    }
    x = (REAL) strtod(stringptr, &stringptr);
    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      my_printf("Error:  Point %d has no y coordinate.\n", firstnumber + i);
      exit(1);
    }
  ...
  if (*holes > 0) {
    ...
    *hlist = holelist;
    ...
    for (i = 0; i < 2 * *holes; i += 2) {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        my_printf("Error:  Hole %d has no x coordinate.\n",
               firstnumber + (i >> 1));
        exit(1);
      } else {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        my_printf("Error:  Hole %d has no y coordinate.\n",
               firstnumber + (i >> 1));
        exit(1);
      } else {
    ...
    if (*regions > 0) {
      ...
      *rlist = regionlist;
      ...
      for (i = 0; i < *regions; i++) {
        ...
        stringptr = findfield(stringptr);
        if (*stringptr == '\0') {
          my_printf("Error:  Region %d has no x coordinate.\n",
                 firstnumber + i);
          exit(1);
        } else {
        ...
        stringptr = findfield(stringptr);
        if (*stringptr == '\0') {
          my_printf("Error:  Region %d has no y coordinate.\n",
                 firstnumber + i);
          exit(1);
        } else {
          regionlist[index++] = (REAL) strtod(stringptr, &stringptr);
...

{
  ...
  if (outfile == (FILE *) NULL) {
    my_printf("  Error:  Cannot create file %s.\n", nodefilename);
    exit(1);
  }
...
#ifdef TRILIBRARY
  if (!quiet) {
    my_printf("Writing Voronoi vertices.\n");
  }
...
#ifdef TRILIBRARY
  if (!quiet) {
    my_printf("Writing Voronoi edges.\n");
  }
...
void quality_statistics()
{
  ...
  my_printf("Mesh quality statistics:\n\n");
  radconst = PI / 18.0;
  degconst = 180.0 / PI;
  for (i = 0; i < 8; i++) {
  ...
  my_printf("  Smallest area: %16.5g   |  Largest area: %16.5g\n",
         smallestarea, biggestarea);
  my_printf("  Shortest edge: %16.5g   |  Longest edge: %16.5g\n",
         shortest, longest);
  my_printf("  Shortest altitude: %12.5g   |  Largest aspect ratio: %8.5g\n\n",
         minaltitude, worstaspect);
  my_printf("  Aspect ratio histogram:\n");
  my_printf("  1.1547 - %-6.6g    :  %8d    | %6.6g - %-6.6g     :  %8d\n",
         ratiotable[0], aspecttable[0], ratiotable[7], ratiotable[8],
         aspecttable[8]);
  for (i = 1; i < 7; i++) {
    my_printf("  %6.6g - %-6.6g    :  %8d    | %6.6g - %-6.6g     :  %8d\n",
           ratiotable[i - 1], ratiotable[i], aspecttable[i],
           ratiotable[i + 7], ratiotable[i + 8], aspecttable[i + 8]);
  }
  my_printf("  %6.6g - %-6.6g    :  %8d    | %6.6g -            :  %8d\n",
         ratiotable[6], ratiotable[7], aspecttable[7], ratiotable[14],
         aspecttable[15]);
  my_printf(
"  (Triangle aspect ratio is longest edge divided by shortest altitude)\n\n");
  my_printf("  Smallest angle: %15.5g   |  Largest angle: %15.5g\n\n",
         smallestangle, biggestangle);
  my_printf("  Angle histogram:\n");
  for (i = 0; i < 9; i++) {
    my_printf("    %3d - %3d degrees:  %8d    |    %3d - %3d degrees:  %8d\n",
...
{
  my_printf("\nStatistics:\n\n");
  my_printf("  Input points: %d\n", inpoints);
  if (refine) {
    my_printf("  Input triangles: %d\n", inelements);
  }
  if (poly) {
    my_printf("  Input segments: %d\n", insegments);
    if (!refine) {
      my_printf("  Input holes: %d\n", holes);
    }
  }

  my_printf("\n  Mesh points: %ld\n", points.items);
  my_printf("  Mesh triangles: %ld\n", triangles.items);
  my_printf("  Mesh edges: %ld\n", edges);
  if (poly || refine) {
    my_printf("  Mesh boundary edges: %ld\n", hullsize);
    my_printf("  Mesh segments: %ld\n\n", shelles.items);
  } else {
    my_printf("  Mesh convex hull edges: %ld\n\n", hullsize);
  }
  if (verbose) {
    quality_statistics();
    my_printf("Memory allocation statistics:\n\n");
    my_printf("  Maximum number of points: %ld\n", points.maxitems);
    my_printf("  Maximum number of triangles: %ld\n", triangles.maxitems);
    if (shelles.maxitems > 0) {
      my_printf("  Maximum number of segments: %ld\n", shelles.maxitems);
    }
    if (viri.maxitems > 0) {
      my_printf("  Maximum number of viri: %ld\n", viri.maxitems);
    }
    if (badsegments.maxitems > 0) {
      my_printf("  Maximum number of encroached segments: %ld\n",
             badsegments.maxitems);
    }
    if (badtriangles.maxitems > 0) {
      my_printf("  Maximum number of bad triangles: %ld\n",
             badtriangles.maxitems);
    }
    if (splaynodes.maxitems > 0) {
      my_printf("  Maximum number of splay tree nodes: %ld\n",
             splaynodes.maxitems);
    }
    my_printf("  Approximate heap memory use (bytes): %ld\n\n",
    ...
    my_printf("Algorithmic statistics:\n\n");
    my_printf("  Number of incircle tests: %ld\n", incirclecount);
    my_printf("  Number of orientation tests: %ld\n", counterclockcount);
    if (hyperbolacount > 0) {
      my_printf("  Number of right-of-hyperbola tests: %ld\n",
             hyperbolacount);
    }
    if (circumcentercount > 0) {
      my_printf("  Number of circumcenter computations: %ld\n",
             circumcentercount);
    }
    if (circletopcount > 0) {
      my_printf("  Number of circle top computations: %ld\n",
             circletopcount);
    }
    my_printf("\n");
  }
...

{
  ...
  if (!quiet) {
    gettimeofday(&tv2, &tz);
    if (refine) {
      my_printf("Mesh reconstruction");
    } else {
      my_printf("Delaunay");
    }
    my_printf(" milliseconds:  %ld\n", 1000l * (tv2.tv_sec - tv1.tv_sec)
  ...
  if (!quiet) {
    ...
    if (useshelles && !refine) {
      my_printf("Segment milliseconds:  %ld\n",
...
/*printf("ZZZZZZ hack in place, please remove\n");*/
  if (0 && poly) {
#ifdef TRILIBRARY
  ...
  if (!quiet) {
    gettimeofday(&tv4, &tz);
    if (poly && !refine) {
      my_printf("Hole milliseconds:  %ld\n", 1000l * (tv4.tv_sec - tv3.tv_sec)
...
#ifndef CDT_ONLY
    if (quality) {
      my_printf("Quality milliseconds:  %ld\n",
  ...
  if (nonodewritten || (noiterationnum && readnodefile)) {
    if (!quiet) {
#ifdef TRILIBRARY
      my_printf("NOT writing points.\n");
#else /* not TRILIBRARY */
      my_printf("NOT writing a .node file.\n");
#endif /* not TRILIBRARY */
    }
  ...
  if (noelewritten) {
    if (!quiet) {
#ifdef TRILIBRARY
      my_printf("NOT writing triangles.\n");
#else /* not TRILIBRARY */
      my_printf("NOT writing an .ele file.\n");
#endif /* not TRILIBRARY */
    }
  } else {
#ifdef TRILIBRARY
  ...
  if (poly || convex) {
    ...
    if (nopolywritten || noiterationnum) {
      if (!quiet) {
#ifdef TRILIBRARY
        my_printf("NOT writing segments.\n");
#else /* not TRILIBRARY */
        my_printf("NOT writing a .poly file.\n");
#endif /* not TRILIBRARY */
      }
    } else {
#ifdef TRILIBRARY
  ...
  if (!quiet) {
    ...
    my_printf("Total running milliseconds:  %ld\n",
