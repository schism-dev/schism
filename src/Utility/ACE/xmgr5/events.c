/* $Id: events.c,v 1.3 2004/06/16 18:34:08 pturner Exp $
 ...
 */
    ...
    case PLACE_VSCALE:
	set_cursor(0);
	set_left_footer("Click at the location for the velocity legend");
	break;
    case PLACE_ISOLINES_LEGEND:
	set_cursor(0);
	set_left_footer("Click at the location for the isolines legend");
	break;
    case PICK_SET:
    case PICK_EXPR:
    case PICK_HISTO:
 ...
 */
		...
		break;
/*
 * place the velocity legends
 */
	    case PLACE_VSCALE:
		{
		    device2world(x, y, &wx, &wy);
		    if (g[cg].vp.loctype == VIEW) {
			wx = xconv(wx);
			wy = yconv(wy);
		    }
		    g[cg].vp.velx = wx;
 ...
 */
	...
	case Button2:
	    ...
	    case TRACKER:
		{
		    ...
		    if (track_set == -1) {
			device2world((int) x, (int) y, &wx, &wy);
			findpoint(cg, wx, wy, &wx, &wy, &track_set, &track_point);
			track_point--;
			if (track_point < 0) {
			    track_point = getsetlength(cg, track_set) - 1;
