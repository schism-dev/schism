/* $Id: defines.h,v 1.2 2006/03/03 00:26:14 pturner Exp $
 *
...
#define GR_GIFP		5	/* mif landscape */
#define GR_GIFL		6	/* mif portrait */
...
#define PLACE_ISOLINES_LEGEND 101
#define PLACE_VSCALE 102
/* for quality control code for editing data */
#define QUALITY_DELETE_POINT 105
#define QUALITY_UNDELETE_POINT 106
#define QUALITY_QUERY_POINT 107
#define QUALITY_SET_FLAG 108
...
typedef struct _CDFMonitor {
    int type;
    int active;
    char fname[1024];
    int var; /* variable name */
    int gno; /* graph */
    int setno; /* set */
    int load; /* overwrite or append */
    int begin; /* begin at end of start of file */
    int autoscale; /* autoscale at each time step */
    int window; /* window the data  (X, Y, XY) */
    double startx; /* start of window in X*/
    double stopx; /* end of window in X*/
    double starty; /* start of window in Y*/
    double stopy; /* end of window in Y*/
    size_t prev;
    size_t cur;
} CDFMonitor;
...
typedef struct {
    ...
    int prec[MAXAXES];		/* save tick label precision (PL3) */
} world_stack;
...
typedef struct {
    ...
    int len;			/* set length */
    int hasmissing;		/* missing data flag */
    ...
    Spline *spl;		/* coefs from any spline fit performed on this set */
    void *ep;			/* pointer to EditPoints structure */
    int nx;			/* Number of points in x direction */
    int ny;			/* Number of points in 7 direction */
    int nelem;			/* Number of elements */
    int *nnodes;		/* pointer to number of nodes per element */
    int *elements[4];		/* pointers to table of elements */
} plotarr;
