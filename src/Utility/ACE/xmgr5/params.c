/* $Id: params.c,v 1.2 2004/07/07 04:00:27 pturner Exp $
 *
...
*/
	    ...
	    get_graph_velocityp(gno, &vp);
	    fprintf(pp, "%s    velocity legend %s\n", imbedstr, on_or_off(vp.active));
	    fprintf(pp, "%s    velocity scale %lf\n", imbedstr, vp.vscale);
	    fprintf(pp, "%s    velocity legend length %lf\n", imbedstr, vp.userlength);
	    fprintf(pp, "%s    velocity legend xy %lf, %lf\n", imbedstr, vp.velx, vp.vely);
	    fprintf(pp, "%s    velocity legend label \"%s\"\n", imbedstr, vp.vstr.s);
	    fprintf(pp, "%s    velocity linewidth %d\n", imbedstr, vp.linew);
	    fprintf(pp, "%s    velocity linestyle %d\n", imbedstr, vp.lines);
	    fprintf(pp, "%s    velocity color %d\n", imbedstr, vp.color);
	    fprintf(pp, "%s    velocity loctype %s\n", imbedstr, w_or_v(vp.loctype));
	    fprintf(pp, "%s    velocity arrow type %d\n", imbedstr, vp.arrowtype);

/* write isoline settings */
	    if (g[gno].isol.nisol > 0) {
	        fprintf(pp, "%s    isolines %d\n", imbedstr, g[gno].isol.nisol);
	        fprintf(pp, "%s    isolines type %d\n", imbedstr, g[gno].isol.isoltype);
	        fprintf(pp, "%s    isolines legend %s\n", imbedstr, 
				on_or_off(g[gno].isol.lactive));
	        fprintf(pp, "%s    isolines legend loctype %s\n", imbedstr, 
				w_or_v(g[gno].isol.loctype));
	        fprintf(pp, "%s    isolines legend layout %s\n", imbedstr, 
				g[gno].isol.layout == HORIZONTAL ? "horizontal" : "vertical");
	        fprintf(pp, "%s    isolines legend %lf, %lf\n", imbedstr, 
				g[gno].isol.x, g[gno].isol.y);
	        fprintf(pp, "%s    isolines legend label format %s\n", imbedstr, 
				get_format_types(g[gno].isol.p.format));
	        fprintf(pp, "%s    isolines legend label prec %d\n", imbedstr, 
				g[gno].isol.p.prec);
	        fprintf(pp, "%s    isolines legend label size %lf\n", imbedstr, 
				g[gno].isol.p.charsize);
	        fprintf(pp, "%s    isolines legend size %lf, %lf\n", imbedstr, 
				g[gno].isol.xlen, g[gno].isol.ylen);
	        fprintf(pp, "%s    isolines legend hgap %lf, %lf\n", imbedstr, 
				g[gno].isol.xgap, g[gno].isol.ygap);
	        fprintf(pp, "%s    isolines start %lf step %lf\n", imbedstr, 
				g[gno].isol.cis[0], g[gno].isol.cint);
	        for (i=0; i<g[gno].isol.nisol; i++) {
	            fprintf(pp, "%s    isoline %d, %lf\n", 
				imbedstr, i, g[gno].isol.cis[i]);
	        }
	    }
	}
    }
