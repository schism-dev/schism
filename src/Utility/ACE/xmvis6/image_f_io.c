 ...
 */
save_image_on_disk(disp, info_win_id, win_id, x, y, width, height, name_of_file, the_colormap)
Display *disp;
Window info_win_id;
Pixmap win_id;
 ...
 *            RGB values.
 */
Get_Colors(colors, disp, the_colormap)
XColor **colors;
Display *disp;
Colormap the_colormap;
{
 ...
 */
XImage *read_gif(disp, win_id, name_of_file, width, height, depth, pflag)
Display *disp;
Window win_id;
char *name_of_file;
int *width, *height, *depth, pflag;
{
    XImage *ImagePix, *LoadGIF();
    ImagePix = LoadGIF(name_of_file);
    if (ImagePix != NULL) {
...
XImage *read_image_from_disk(disp, win_id, name_of_file, width, height, depth, pflag)
Display *disp;
Window win_id;
char *name_of_file;
int *width, *height, *depth, pflag;
{
	...
	fclose(in_file);
	sprintf(pipename, "zcat %s", name_of_file);

	if ((in_file = (FILE *) popen(pipename, "r")) == NULL) {	/* open file for read */
	    return (0);
	}
    } else {
    ...
    if (pflag) {
	(void) pclose(in_file);	/* we are done with the infile */
    } else {
 ...
 */
allocate_colors_and_assign_em(disp, win_id, image_ptr, cells)
Display *disp;
Window win_id;
XImage **image_ptr;
XColor *cells;
{
	 ...
	 * pixels.
	 */
	diff_depth_im = XCreateImage(disp, win_attr.visual, win_attr.depth, image->format, image->xoffset, NULL, width, height, image->bitmap_pad, 0);
...
XImage *MakeImage(dsp, data, width, height, depth)
Display *dsp;
unsigned char *data;
int width, height;
int depth;
{
    ...
    bytesperline = width;
    newimage = XCreateImage(dsp, DefaultVisual(dsp, DefaultScreen(dsp)), depth, ZPixmap, 0, (char *) bit_data, width, height, 8, bytesperline);
    return newimage;
}
