...
!-------------------------------------------------------------------- 
      ...
      udir = DATAN2(vstress(inea),ustress(inea))
...
!--------------------------------------------------------------------
      theta_max1 = SQRT((theta_m+cff1*dcos(phi))**2+(cff1*dsin(phi))**2)
      theta_max2 = SQRT((theta_m+cff2*dcos(phi+pi))**2+(cff2*dsin(phi+pi))**2)
...
!-------------------------------------------------------------------------
      ...
      phi_x2 = 12.0d0 * (0.9534d0 + 0.1907d0*dcos(2.d0*phi))*sqrt(theta_w)*theta_m + 12.0d0*(0.229d0*w_asym*theta_w**1.5d0*dcos(phi))
...
!--------------------------------------------------------------------
      ...
      phi_y=12.0d0*0.1907d0*theta_w**2.0d0*(theta_m*dsin(2.0d0*phi) + 1.2d0*w_asym*theta_w*dsin(phi))/cff4
      IF (phi_y /= phi_y) THEN
        bedld_y = 0.d0
      ELSE
...
!      FY_r(inea) = sqrt(bedld_x**2.d0+bedld_y**2.d0)*dsin(udir)*dt
      FX_r(inea) = (bedld_x*dcos(udir)-bedld_y*dsin(udir))*dt
      FY_r(inea) = (bedld_y*dcos(udir)+bedld_x*dsin(udir))*dt
